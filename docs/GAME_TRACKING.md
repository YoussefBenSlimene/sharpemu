...
| M34 | **P0: 1 present / 300 s after the mutex fast path (`1035a7d`), was ~2,300 at `3a00b0f`** | **FIX (pending Windows A/B)** | Code review of `1035a7d`: the native `FastMutexUnlock` calls `PthreadMutexState.TryReleaseUncontended`, which reads `QueuedWaiterCount == 0` and then CAS-clears the owner, all **without `SyncRoot`**. A contender on the managed path that found the mutex owned, took `SyncRoot` and enqueued itself between those two steps saw `owner != 0`, failed `TryGrantMutexWaiterLocked` and parked, and nobody woke it: `WaitForHostMutexLock` parks on an **untimed** `ManualResetEventSlim`, and cooperative waiters wait on a wake key. The managed unlock always re-checks `QueuedWaiterCount` after the same release and hands off; the native unlock skipped that step, so an unlucky interleaving stranded a render or loader thread for good. Fix: after a successful native release, if a waiter is queued, run the same hand-off as the managed unlock (`GrantHeadMutexWaiter`: grant the head waiter under `SyncRoot`, then signal it or wake it cooperatively). `5e0b737` (loop-guard transparency plus the error-check self-lock answer), `b3003b0` (loop guard opt-in) and `3a00b0f` (harness env) were reviewed and ruled out: none of them touches wake-ups. Test: `NativeUnlock_NeverStrandsAManagedWaiter` (3 native + 3 managed contenders, 4 s, mutual exclusion plus progress; 865/865). The race window is a few instructions, so the unit test did **not** reproduce the stall without the fix; the in-game A/B is the proof | this commit |
...
| S6 | Unresolved imports (~188 calls each) from eboot `0x8032F98xx`–`0x8033020xx` (UE online subsystem): `sceNpWebApi2CheckTimeout` (`3Tt9zL3tkoc`) + four unresolvable NIDs (`7hd4bRJuLMg`, `Cm2cmtCv8cA`, `xaNK0ZTH3QA`, `NDMQ5Z8QdWQ`) | **PARTIAL FIX** | Ported the KytyPS5 `libNet.cpp` offline LibNpWebApi2 implementation: CheckTimeout, CreateRequest/SendRequest (SendRequest answers NOT_SIGNED_IN `0x80553407`, unknown ids answer REQUEST_NOT_FOUND), ReadData (0 bytes), Abort/DeleteRequest, AddHttpRequestHeader, response-header length/value (Content-Type json), PushEvent register/delete, DeleteUserContext. The online thread now gets a clean "offline" answer instead of NOT_FOUND. The remaining unknown NIDs (e.g. `Cm2cmtCv8cA`, which is not in Kyty either) stay unresolved, but the warning is now rate-limited per NID (first 4, then powers of 2, with `count=`) so it no longer floods stderr during loading. Same commit: GAME-DBG draw/compute/present traces format their strings only after the rate limiter admits them (they used to build per draw even when suppressed, a load-time allocation source) | this commit |
