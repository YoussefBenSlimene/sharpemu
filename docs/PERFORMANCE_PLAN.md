...
- **Phase C.1 (first slice): zero-timeout `cond_timedwait` fast path.** A timed wait that has already expired (relative 0 µs, or an absolute `pthread_cond_timedwait` deadline in the past) now returns TIMED_OUT (ETIMEDOUT for the POSIX export) immediately and keeps the mutex, as FreeBSD umtx and Kyty `cond_cv.wait_for(0)` do. Before this, every such poll paid for the waiter allocation, a string wake key, the unlock, the scheduler block/resume and the FIFO re-lock. A/B switch: `SHARPEMU_DISABLE_COND_ZERO_TIMEOUT_FASTPATH=1`. Test: `CondTimedwait_ZeroTimeoutReturnsTimedOutAndKeepsTheMutex`. Metric: `bench_run.ps1 mortal 600`, compare imports/s and pak reads against the baseline row.
- Next: non-zero timed waits. Replace the per-wait `Timer` with the scheduler deadline (`blockDeadlineTimestamp`, already supported by `RequestCurrentThreadBlock`) and drop the per-wait string wake key. Then an allocation-source audit.
